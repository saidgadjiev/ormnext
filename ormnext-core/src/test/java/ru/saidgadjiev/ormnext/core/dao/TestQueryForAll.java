 ...
 */
public class TestQueryForAll extends BaseCoreTest {

    @Test
    public void test() throws SQLException {
        try (Session session = createSessionAndCreateTables(AForeignCollection.class, A.class)) {
            AForeignCollection foreignCollectionTestEntity = new AForeignCollection();

            session.create(foreignCollectionTestEntity);
            A entity = new A();

            entity.setDesc("Test");
            entity.setaForeignCollection(foreignCollectionTestEntity);
            session.create(entity);

            A entity1 = new A();

            entity1.setDesc("Test1");
            entity1.setaForeignCollection(foreignCollectionTestEntity);

            session.create(entity1);

            List<A> result = session.queryForAll(A.class);

            Assert.assertEquals(result.size(), 2);
            Assert.assertEquals(result.get(0), entity);
            Assert.assertEquals(result.get(1), entity1);
            Assert.assertEquals(result.get(0).getaForeignCollection(), foreignCollectionTestEntity);
            Assert.assertEquals(result.get(1).getaForeignCollection(), foreignCollectionTestEntity);
        }
    }
