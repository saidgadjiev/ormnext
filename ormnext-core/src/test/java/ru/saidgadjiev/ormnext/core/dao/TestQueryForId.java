 ...
 */
public class TestQueryForId extends BaseCoreTest {

    @Test
    public void test() throws SQLException {
        try (Session session = createSessionAndCreateTables(AForeignCollection.class, A.class)) {
            AForeignCollection entity = new AForeignCollection();

            entity.setDesc("Test");
            session.create(entity);
            A foreignTestEntity = new A();

            foreignTestEntity.setaForeignCollection(entity);
            session.create(foreignTestEntity);

            A result = session.queryForId(A.class, 1);

            Assert.assertEquals(foreignTestEntity, result);
            Assert.assertEquals(foreignTestEntity.getaForeignCollection(), result.getaForeignCollection());
            Assert.assertEquals(result.getaForeignCollection().getAs().size(), 1);
            Assert.assertEquals(result.getaForeignCollection().getAs().get(0), foreignTestEntity);
        }
    }
